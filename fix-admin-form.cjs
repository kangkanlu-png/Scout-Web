...
let startIndex = file.indexOf('function activityForm(activity: any) {');
let nextFuncIndex = file.indexOf('// ===================== 公告管理 =====================', startIndex);
if (nextFuncIndex === -1) {
    nextFuncIndex = file.indexOf('// ===================== 活動報名管理', startIndex); // Try to find the next section
}
if (nextFuncIndex === -1) {
    nextFuncIndex = file.indexOf('function imageUploadForm', startIndex);
}

const formStr = `function activityForm(activity: any) {
...
file = file.substring(0, startIndex) + formStr + file.substring(nextFuncIndex);
