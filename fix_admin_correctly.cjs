const fs = require('fs');
let adm = fs.readFileSync('src/routes/admin.tsx', 'utf8');

// Replace /activities get route logic safely
adm = adm.replace(/adminRoutes\.get\('\/activities', authMiddleware, async \(c\) => \{[\s\S]*?return c\.html\(adminLayout\('活動管理', `([\s\S]*?)`\)\)\n\}\)/, 
...
let formStart = adm.indexOf('function activityForm(activity: any) {');
let formEnd = adm.indexOf('function groupFormFields', formStart);
let newForm = `function activityForm(activity: any) {
...
`;
adm = adm.substring(0, formStart) + newForm + adm.substring(formEnd);

adm = adm.replace(/<span>📋<\/span> 活動管理/, '<span>📋</span> 活動/公告管理');
adm = adm.replace(/<span>📢<\/span> 公告管理/, '<!-- <span>📢</span> 公告管理 (已整合) -->');

fs.writeFileSync('src/routes/admin.tsx', adm);
